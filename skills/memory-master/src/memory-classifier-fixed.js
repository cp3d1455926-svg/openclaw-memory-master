...
class MemoryTypeClassifier {
  ...
  classify(content, context = {}) {
    ...
    // 1. 特征词评分
    this.scoreByFeatures(content, scores);
  ...
  scoreByFeatures(content, scores) {
    ...
    for (const [type, config] of Object.entries(ClassificationFeatures)) {
...
module.exports = {
  MemoryTypes,
  MemoryTypeConfig,
  ClassificationFeatures,  // 修复：导出新变量名
