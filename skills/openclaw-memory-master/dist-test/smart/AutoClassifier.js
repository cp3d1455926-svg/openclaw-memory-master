...
exports.AutoClassifier = exports.DEFAULT_CLASSIFICATION_RULES = void 0;
exports.createClassifier = createClassifier;
exports.quickClassify = quickClassify;
// ============ 规则定义 ============
/**
 * 预定义分类规则
 * 基于关键词匹配的快速分类
...
class AutoClassifier {
    constructor(config) {
        this.config = config;
        this.rules = exports.DEFAULT_CLASSIFICATION_RULES.filter(rule => config.categories.includes(rule.category));
     ...
     * 规则分类
     */
    async ruleBasedClassify(content) {
     ...
     * LLM分类
     */
    async llmClassify(content, ruleResult) {
        const startTime = Date.now();
        try {
            // 构建LLM请求
            const request = {
     ...
     * 调用LLM进行分类（占位实现）
     */
    async callLLMForClassification(request) {
     ...
     * 计算最大可能得分
     */
    calculateMaxPossibleScore(content) {
        // 简单实现：基于内容长度和规则数量
     ...
     * 归一化置信度
     */
    normalizeConfidence(rawConfidence, contentLength) {
        // 考虑内容长度的置信度调整
...
exports.AutoClassifier = AutoClassifier;
/**
 * 导出工具函数
 */
 ...
 * 快速分类函数
 */
async function quickClassify(content, config) {
