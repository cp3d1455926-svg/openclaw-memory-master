...
exports.DeduplicationEngine = exports.DEFAULT_SIMILARITY_THRESHOLDS = void 0;
exports.createDeduplicationEngine = createDeduplicationEngine;
exports.quickDeduplicateCheck = quickDeduplicateCheck;
// ============ 常量定义 ============
/**
 * 默认相似度阈值
 */
        ...
        for (const memory of sortedMemories) {
            ...
            try {
                ...
                for (const uniqueMemory of uniqueMemories) {
                    const similarity = await this.calculateSimilarity(memory.content, uniqueMemory.content);
...
`;
     ...
     * 检查语义相似度
     */
    async checkSemanticSimilarity(memory) {
        // 简化实现：使用文本相似度计算
        // 实际应该使用向量嵌入
        let bestMatch = null;
        for (const [hash, existingId] of this.memoryIndex.entries()) {
    ...
    normalizeText(text) {
        ...
        if (this.config.semanticCheck) {
            ...
            const filteredWords = words.filter(word => word.length > 1 && !STOP_WORDS.has(word));
 ...
 * 导出工具函数
 */
function createDeduplicationEngine(config) {
    const fullConfig = {
 ...
 * 快速去重检查函数
 */
async function quickDeduplicateCheck(memory, config) {
