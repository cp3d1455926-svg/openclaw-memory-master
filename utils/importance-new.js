...
const SCORING_CONFIG = {
  ...
  // 用户明确指令（+30 分）
  explicitCommands: new RegExp('(记住 | 记下来 | 别忘了 | 记录 | 保存 | 写到记忆 | 添加到记忆 | 务必 | 一定)'),
  
  // 日期格式（+8 分）
  datePattern: new RegExp('(\\\\d{4}-\\\\d{2}-\\\\d{2}|\\\\d{1,2}月\\\\d{1,2}日 | 今天 | 明天 | 后天 | 昨天 | 下周 | 上周 | 本周)'),
  ...
  // 总结性语句（+20 分）
  summaryWords: new RegExp('(总之 | 所以 | 决定 | 结论 | 综上所述 | 总的来说 | 简而言之 | 最终 | 确定 | 敲定)'),
  ...
  // 人名/称呼（+8 分）
  namePattern: new RegExp('(@[\\w]+|[A-Z][a-z]+ [A-Z][a-z]+|先生 | 女士 | 同学 | 老师 | 朋友 | 家人|Jake|Alice|Bob)'),
 ...
 */
const WEIGHTS = {
  keywords: 10,      // 每个关键词 10 分，上限 30 分
  explicitCommands: 30,  // 用户明确指令
  date: 8,           // 日期
  url: 8,            // URL
  number: 5,         // 数字
  file: 8,           // 文件名
...
function calculateImportance(message, options = {}) {
  ...
  if (keywordCount > 0) {
    const points = Math.min(keywordCount * WEIGHTS.keywords, 30); // 上限 30 分
    score += points;
