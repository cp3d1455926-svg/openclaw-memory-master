...
export interface RelationConfig {
  ...
  entityWeight: number;                 // 实体共现权重
  temporalWeight: number;               // 时间相近权重
  semanticWeight: number;               // 语义相似权重
  categoryWeight: number;               // 分类相似权重
  maxRelatedMemories: number;           // 最大相关记忆数
  enableCausalInference: boolean;       // 是否启用因果推断
}

export interface RelationResult {
  relatedIds: string[];                 // 相关记忆ID
  relationTypes: string[];              // 关系类型
  relationStrengths: number[];          // 关系强度 0-1
  details: {
    entityMatches: Array<{ id: string; entities: string[]; matchScore: number }>;
    temporalMatches: Array<{ id: string; timeDiffHours: number; matchScore: number }>;
    semanticMatches: Array<{ id: string; similarity: number; matchScore: number }>;
    categoryMatches: Array<{ id: string; category: string; matchScore: number }>;
  };
  explanation: string;                  // 关系发现解释
}
...
export interface TemporalAnalysis {
  ...
  hourOfDay: number;                    // 一天中的小时
  dayOfWeek: number;                    // 一周中的天
  month: number;                        // 月份
 ...
 * 关系强度映射
 */
export const RELATION_STRENGTH_MAPPING = {
  weak: 0.3,     // 弱关系
  moderate: 0.6,  // 中等关系
  strong: 0.85,  // 强关系
...
export class RelationDiscoverer {
  ...
  private memoryRegistry: Map<string, RawMemory>; // 记忆注册表（简化）
  private statistics: {
  ...
  constructor(config?: Partial<RelationConfig>) {
    this.config = { ...DEFAULT_RELATION_CONFIG, ...config };
    ...
    // 初始化关系类型统计
    Object.values(RELATION_TYPES).forEach(type => {
   ...
   * 发现记忆关系
   */
  async discoverRelations(memory: RawMemory): Promise<{ 
    relatedIds: string[]; 
    ...
    try {
      ...
      for (const [memId, existingMemory] of this.memoryRegistry.entries()) {
        ...
        const matchResult = await this.compareFeatures(
          currentFeatures,
          existingFeatures,
          memory,
          existingMemory
      ...
      const relationTypes = Array.from(new Set(
        topMatches.flatMap(match => match.relationTypes)
      ));
   ...
   * 批量发现关系（增强版）
   */
  async discoverRelationsEnhanced(memory: RawMemory): Promise<RelationResult> {
    const startTime = Date.now();
    ...
    try {
      ...
      const entityMatches: Array<{ id: string; entities: string[]; matchScore: number }> = [];
      const temporalMatches: Array<{ id: string; timeDiffHours: number; matchScore: number }> = [];
      const semanticMatches: Array<{ id: string; similarity: number; matchScore: number }> = [];
      const categoryMatches: Array<{ id: string; category: string; matchScore: number }> = [];
      ...
      for (const [memId, existingMemory] of this.memoryRegistry.entries()) {
        ...
        const existingFeatures = await this.extractFeatures(existingMemory);
        const comparison = await this.detailedCompare(
          currentFeatures,
          existingFeatures,
          memory,
          existingMemory
...
`;
   ...
   * 提取记忆特征
   */
  private async extractFeatures(memory: RawMemory): Promise<{
   ...
   * 提取实体
   */
  private extractEntities(content: string): EntityExtractionResult {
    // 简化实体提取，实际应该使用NLP库
   ...
   * 分析时间特征
   */
  private analyzeTemporal(memory: RawMemory): TemporalAnalysis {
  ...
  private async compareFeatures(
    current: any,
    existing: any,
    currentMemory: RawMemory,
    existingMemory: RawMemory
  ): Promise<{
    relationTypes: string[];
    strength: number;
    details: any;
    ...
    if (entityScore > 0) {
      relationTypes.push(RELATION_TYPES.ENTITY_COOCCURRENCE);
    ...
    if (temporalScore > 0) {
      relationTypes.push(RELATION_TYPES.TEMPORAL_PROXIMITY);
    ...
    if (semanticScore > 0) {
      relationTypes.push(RELATION_TYPES.SEMANTIC_SIMILARITY);
    ...
    if (categoryScore > 0) {
      relationTypes.push(RELATION_TYPES.CATEGORY_SIMILARITY);
    ...
    if (this.config.enableCausalInference) {
      const causalScore = await this.inferCausalRelation(
        currentMemory,
        existingMemory,
        current,
        existing
      ...
      if (causalScore > 0) {
        relationTypes.push(RELATION_TYPES.CAUSAL_RELATION);
  ...
  private async detailedCompare(
    ...
    // 确定关系类型
    const relationTypes: string[] = [];
    if (entityScore > 0.3) relationTypes.push(RELATION_TYPES.ENTITY_COOCCURRENCE);
    if (temporalScore > 0.3) relationTypes.push(RELATION_TYPES.TEMPORAL_PROXIMITY);
    if (semanticScore > 0.3) relationTypes.push(RELATION_TYPES.SEMANTIC_SIMILARITY);
    if (categoryScore > 0.3) relationTypes.push(RELATION_TYPES.CATEGORY_SIMILARITY);
    
    // 逻辑关联推断
    if (semanticScore > 0.6 || entityScore > 0.5) {
      relationTypes.push(RELATION_TYPES.LOGICAL_ASSOCIATION);
    }
    ...
    if (current.emotions && existing.emotions) {
      ...
      if (emotionalOverlap > 0.5) {
        relationTypes.push(RELATION_TYPES.EMOTIONAL_CONNECTION);
      }
    }
    
    // 主题相关
    if (current.entities.topics.some(topic => existing.entities.topics.includes(topic))) {
      relationTypes.push(RELATION_TYPES.THEMATIC_RELATION);
    }
    ...
    const overallScore = (
      ...
      this.config.entityWeight + this.config.temporalWeight + 
  ...
  private calculateEntitySimilarity(
    entities1: EntityExtractionResult,
    entities2: EntityExtractionResult
  ): number {
    ...
    // 检查所有实体类别
    const categories = ['persons', 'locations', 'organizations', 'topics', 'events', 'dates'] as const;
  ...
  private calculateTemporalSimilarity(
    temporal1: TemporalAnalysis,
    temporal2: TemporalAnalysis
  ): number {
   ...
   * 计算分类相似度
   */
  private calculateCategorySimilarity(category1?: string, category2?: string): number {
   ...
   * 推断因果关系
   */
  private async inferCausalRelation(
    currentMemory: RawMemory,
    existingMemory: RawMemory,
    currentFeatures: any,
    ...
    // 检查实体连续性
    const entityContinuity = this.calculateEntitySimilarity(
      currentFeatures.entities,
      existingFeatures.entities
    );
   ...
   * 计算情感重叠
   */
  private calculateEmotionalOverlap(emotions1: string[], emotions2: string[]): number {
   ...
   * 获取关系类型名称
   */
  private getRelationTypeName(relationType: string): string {
    const nameMap: Record<string, string> = {
      [RELATION_TYPES.ENTITY_COOCCURRENCE]: '实体共现',
      [RELATION_TYPES.TEMPORAL_PROXIMITY]: '时间相近',
      [RELATION_TYPES.SEMANTIC_SIMILARITY]: '语义相似',
      [RELATION_TYPES.CATEGORY_SIMILARITY]: '分类相似',
      [RELATION_TYPES.CAUSAL_RELATION]: '因果关系',
      [RELATION_TYPES.LOGICAL_ASSOCIATION]: '逻辑关联',
      [RELATION_TYPES.EMOTIONAL_CONNECTION]: '情感连接',
      [RELATION_TYPES.THEMATIC_RELATION]: '主题相关',
    };
   ...
   * 更新统计
   */
  private updateStatistics(relationCount: number, relationTypes: string[]): void {
 ...
 * 导出工具函数
 */
export function createRelationDiscoverer(config?: Partial<RelationConfig>): RelationDiscoverer {
 ...
 * 快速关系发现函数
 */
export async function quickDiscoverRelations(
  memory: RawMemory,
  config?: Partial<RelationConfig>
): Promise<{ relatedIds: string[]; relationTypes: string[] }> {
